...
export const OnboardingTour: React.FC<Props> = ({ onComplete, onRequestView, onRequestMode, currentView }) => {
  ...
  const updatePosition = () => {
    ...
    if (step.targetId === 'center' || !hasElement) {
        ...
        const padding = 12; // Increased padding for better breathing room
    ...
    if (isMobile) {
      ...
      const gap = 24;
      const navbarBuffer = 100; // Account for 80px navbar + 20px padding
      ...
      // STRICT CLAMPING: Ensure it NEVER goes off screen or under navbar
      if (top < navbarBuffer) top = navbarBuffer;
      if (top + tooltipH > viewportH - 20) top = viewportH - tooltipH - 20;
    ...
    const initStep = async () => {
        ...
        if (step.targetId !== 'center') {
            ...
            if (el) {
                ...
                if (isMobile) {
                    // Mobile: Place just below navbar (approx 120px down)
                    offset = 120; 
                } else {
