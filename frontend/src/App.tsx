...
const Toast = ({ message, visible, type = 'info' }: { message: string, visible: boolean, type?: ToastType }) => {
  const getStyles = () => {
    switch (type) {
      case 'error': return 'border-red-500/30 shadow-[0_0_30px_rgba(239,68,68,0.3)] bg-[#0f0505]/95';
      case 'success': return 'border-green-500/30 shadow-[0_0_30px_rgba(34,197,94,0.3)] bg-[#050f05]/95';
      default: return 'border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.3)] bg-[#050505]/95';
    }
  };
  ...
  return (
    <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-[9999] transition-all duration-500 ${visible ? 'translate-y-0 opacity-100' : '-translate-y-8 opacity-0 pointer-events-none'}`}>
       <div className={`backdrop-blur-xl border px-6 py-3 rounded-full flex items-center gap-3 w-max max-w-[90vw] shadow-2xl ${getStyles()}`}>
          <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${getIconColor()}`}>
             {getIcon()}
          </div>
...
// Reusable SVG Spinner for smooth rotation
const Spinner = () => (
...
function App() {
  ...
  // WAKE UP BACKEND: Ping server on app load to mitigate "Cold Start" delays
  useEffect(() => {
    const warmUpBackend = async () => {
      const url = localStorage.getItem('sentinel_backend_url') || process.env.REACT_APP_BACKEND_URL || 'http://localhost:10000';
      const cleanUrl = url.endsWith('/') ? url.slice(0, -1) : url;
      
      // Add protocol if missing
      const fetchUrl = /^https?:\/\//i.test(cleanUrl) ? cleanUrl : `http://${cleanUrl}`;

      try {
        // Simple ping to wake up Render/Heroku dynos
        await fetch(`${fetchUrl}/`, { method: 'GET' });
        console.log("Sentinel Cloud Engine: Online");
      } catch (e) {
        // Silent fail is fine, user will see status in ThreatHunter component
        console.log("Sentinel Cloud Engine: Sleeping or Offline");
      }
    };
  ...
  const handleTourComplete = () => {
    ...
    triggerToast("You're all set! Stay safe.", 'success');
  };
  ...
  // Auto-scroll to center input when switching modes
  useEffect(() => {
    const scrollToInput = setTimeout(() => {
      // Switch target IDs: Use container wrappers for stability
      const targetId = scannerMode === 'MESSAGE' ? 'tour-input-area' : 'threat-hunter-section';
    ...
    return () => clearTimeout(scrollToInput);
  }, [scannerMode]);
  ...
  const handleManualAnalyze = () => {
    if (inputText.trim().length <= 5) {
        triggerToast("Please enter a complete message to analyze.", 'info');
        return;
    }
    
    setIsAnalyzing(true);
    setResult(null); // Clear previous result while scanning
    ...
    setTimeout(() => {
      try {
        ...
        setIsAnalyzing(false);
        triggerToast("Analysis engine encountered an error. Please try again.", 'error');
      }
    }, 1500); 
  };
  ...
  return (
    <div className={containerClasses} onClick={handleInteraction} onKeyDown={handleInteraction} onTouchStart={handleInteraction}>
      ...
      {/* Navbar */}
      <nav className="w-full fixed top-0 z-50 transition-all duration-300 border-b border-white/5 bg-[#050505]/80 backdrop-blur-xl will-change-transform" role="navigation">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
             ...
             <button 
                ...
                className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-cyan-400 hover:border-cyan-500/30 hover:bg-white/10 transition-all shadow-lg"
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 space-y-12 md:space-y-16 pt-8 relative" role="main">
        ...
        {/* Ambient Main Background - Static for performance on Mobile, Blur reduced */}
        <div className="absolute inset-0 z-[-1] overflow-hidden pointer-events-none rounded-[3rem]">
           <div className="absolute top-[-20%] left-[-10%] w-[70%] h-[70%] bg-blue-900/10 blur-[60px] md:blur-[120px] rounded-full mix-blend-screen" />
           <div className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] bg-purple-900/10 blur-[60px] md:blur-[120px] rounded-full mix-blend-screen" />
           <div className="absolute top-[40%] left-[30%] w-[40%] h-[40%] bg-cyan-900/5 blur-[50px] md:blur-[100px] rounded-full" />
        </div>
        ...
        <div className="flex justify-center pb-8 animate-slide-up w-full scroll-mt-48" id="tour-nav-tabs">
           ...
           >
             <div 
               className={`hidden md:block pointer-events-none absolute top-0 left-0 w-[250px] h-[250px] z-0 transition-opacity duration-300 ${isNavHovered ? 'opacity-100' : 'opacity-0'}`}
               style={{
        ...
        {view === 'home' && (
          <>
            ...
            <section className="space-y-12" aria-label="Scam Detector">
              ...
              {scannerMode === 'THREAT_HUNTER' && (
                <div id="threat-hunter-section" className="scroll-mt-40 transform-gpu">
                  <ThreatHunter onToast={triggerToast} />
                </div>
              )}
              ...
              {scannerMode === 'MESSAGE' && (
              <>
                {/* Static wrapper for Tour Target with extra scroll margin */}
                <div id="tour-input-area" className="scroll-mt-48 block w-full animate-slide-up transform-gpu" style={{ animationDelay: '0.2s' }}>
                        ...
                        <div className="bg-[#0a0a0f]/60 rounded-[2rem] p-6 md:p-12 transition-all duration-500">
                          ...
                          <div className="relative rounded-3xl overflow-hidden">
                            ...
                            <textarea
                              ...
                              className="glass-input w-full min-h-[14rem] h-56 rounded-3xl p-6 md:p-8 pb-24 text-lg md:text-2xl leading-relaxed text-white placeholder:text-slate-600 focus:outline-none transition-all resize-none font-light"
                              spellCheck={false}
                            />
                            
                            {/* Persistent Warning Disclaimer */}
                            <div className="absolute top-20 left-6 right-6 md:left-8 md:right-8 pointer-events-none opacity-40 select-none">
                               <p className="text-[10px] md:text-xs text-amber-500/80 font-mono tracking-wide text-center md:text-left">
                                 ⚠️ AI analysis is an aid, not a guarantee. Always verify urgent requests by calling your family member directly.
                               </p>
                            </div>
                            ...
                            {/* Status Indicator / Placeholder Button */}
                            ...
                            {/* Analysis Controls - Bottom Right */}
                            <div className="absolute bottom-6 right-6 flex items-center gap-4 z-20">
                              {isAnalyzing ? (
                                ...
                                <button 
                                  ...
                                  className={`px-8 py-3 rounded-xl font-bold shadow-lg transition-all transform flex items-center gap-2 text-sm md:text-base tracking-wide ${
                                    inputText.length > 0 
                                      ? 'bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white scale-100 opacity-100 hover:scale-105 shadow-orange-900/20' 
