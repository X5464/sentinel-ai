...
export const AnalysisResultCard: React.FC<Props> = ({ result, onToast }) => {
  ...
  const handleShare = async () => {
    const text = `⚠️ *Sentinel AI Warning* ⚠️\n\nI just scanned a message and Sentinel AI detected a *${result.level} RISK* (${result.score}/100).\n\nDetails: ${result.summary}\n\nStay safe!`;
      ...
      <div className="glass-morphism rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-white/10">
        ...
        <div className="flex flex-col lg:flex-row min-h-[500px]">
          ...
          <div className="w-full lg:w-[28rem] border-b lg:border-b-0 lg:border-r border-white/5 bg-black/20 flex flex-col items-center justify-center relative p-6 md:p-8 overflow-hidden">
            {/* Morphing Liquid Blob Background */}
            <div className={`absolute w-[120%] h-[120%] ${getBlobColor()} blur-[80px] animate-morph opacity-40`} />
            ...
            <div className="space-y-4 relative z-10">
               <div className="flex flex-wrap items-center justify-between gap-4">
                 ...
                 {result.level !== RiskLevel.SAFE && (
                   <button 
                     onClick={handleShare}
                     className="flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white transition-all text-xs md:text-sm font-bold uppercase tracking-wide ml-auto md:ml-0"
                   >
